...
def _normalize_action_infer_mode(action_infer_mode: Any) -> str:
    ...
    if mode not in {"joint_rollout", "one_pass_future_cache"}:
        raise ValueError(
            f"Unsupported EVALUATION.action_infer_mode: {action_infer_mode}. "
...
def _predict_action_chunk(
    ...
    *,
    ...
    action_infer_mode = _normalize_action_infer_mode(
        cfg.EVALUATION.get("action_infer_mode", "joint_rollout")
    )
    predicted_future_frames = None
    if visualize_future_video:
        infer_kwargs["num_video_frames"] = _get_num_video_frames(cfg)
    else:
        infer_action_fn = model.infer_action
        if action_infer_mode == "one_pass_future_cache":
            infer_action_fn = getattr(model, "infer_action_one_pass_future_cache", None)
            if infer_action_fn is None:
                raise ValueError(
                    "EVALUATION.action_infer_mode='one_pass_future_cache' requires model method "
                    "`infer_action_one_pass_future_cache(...)`."
                )
        if "num_video_frames" in inspect.signature(infer_action_fn).parameters:
            infer_kwargs["num_video_frames"] = _get_num_video_frames(cfg)
    ...
    with torch.no_grad():
        if visualize_future_video:
            ...
            pred = infer_action_fn(**infer_kwargs)
