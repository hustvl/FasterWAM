...
def _normalize_action_infer_mode(action_infer_mode: Any) -> str:
    mode = str(action_infer_mode).strip().lower()
    if mode not in {"joint_rollout", "one_pass_future_cache"}:
        raise ValueError(
            f"Unsupported action_infer_mode: {action_infer_mode}. "
            "Expected one of: ['joint_rollout', 'one_pass_future_cache']."
        )
    return mode
...
def _parse_optional_int_list(value: Any, field_name: str) -> Optional[list[int]]:
    if _is_none_like(value):
        return None
    ...
    if isinstance(value, str):
        try:
            ...
            raise ValueError(
                f"`{field_name}` must be a Python-style list of integers, got: {value!r}"
            ) from exc

    if isinstance(parsed, tuple):
        parsed = list(parsed)
    if not isinstance(parsed, list):
        raise ValueError(f"`{field_name}` must be a list of integers, got {type(parsed)}.")

    normalized: list[int] = []
    for item in parsed:
        if isinstance(item, bool) or not isinstance(item, int):
            raise ValueError(f"`{field_name}` must contain only integers, got {item!r}.")
...
class WorldActionRobotWinPolicy:
    def __init__(
        ...
        if self.action_infer_mode == "joint_rollout":
            ...
            self._action_infer_fn = getattr(self.model, "infer_action_one_pass_future_cache", None)
            if self._action_infer_fn is None:
                raise ValueError(
                    "action_infer_mode='one_pass_future_cache' requires model method "
        ...
        logger.info(
            "Initialized WorldActionRobotWinPolicy | ckpt=%s | stats=%s | horizon=%d | replan=%d | infer_mode=%s",
            checkpoint_path,
            dataset_stats_path,
            self.action_horizon,
            self.replan_steps,
            self.action_infer_mode,
        )
...
def get_model(usr_args: Dict[str, Any]):
    ...
    action_infer_mode = _normalize_action_infer_mode(
        usr_args.get("action_infer_mode", cfg.EVALUATION.get("action_infer_mode", "joint_rollout"))
    )
    for key in ("video_kv_fusion", "video_kv_fusion_init"):
        if key in usr_args:
            cfg.model[key] = usr_args[key]
    mot_action_video_attention_layers = _parse_optional_int_list(
        usr_args.get("mot_action_video_attention_layers"),
        field_name="mot_action_video_attention_layers",
    )
    if mot_action_video_attention_layers is not None:
        cfg.model.mot_action_video_attention_layers = mot_action_video_attention_layers
    ...
    policy = WorldActionRobotWinPolicy(
        ...
        action_infer_mode=action_infer_mode,
    )
    return policy
