...
- Check or create the symlink:
  `RoboTwin/policy/fasterwam_policy -> experiments/robotwin/fasterwam_policy`.
- Forward config overrides to the official RoboTwin entrypoint
  `script/eval_policy.py` and save logs.

Common arguments:
- `ckpt`: path to the selected model checkpoint (required).
...
def main(cfg: DictConfig):
    ...
    _append_override(overrides, "action_infer_mode", cfg.EVALUATION.action_infer_mode)
    for key in ("video_kv_fusion", "video_kv_fusion_init"):
        if key in cfg.model:
            _append_override(overrides, key, cfg.model[key], skip_none=False)
    if "mot_action_video_attention_layers" in cfg.model:
        _append_override(
            overrides,
            "mot_action_video_attention_layers",
            cfg.model.mot_action_video_attention_layers,
        )
