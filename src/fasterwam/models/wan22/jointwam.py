...
class JointWAM(FastWAM):
    ...
    def from_wan22_pretrained(cls, **kwargs):
        ...
        if not isinstance(video_dit_config, dict):
            raise ValueError(
                "`video_dit_config` must be provided as dict for JointWAM."
            )
        if bool(video_dit_config.get("action_conditioned", False)):
            raise ValueError(
                "JointWAM requires `video_dit_config['action_conditioned']=false`."
            )
    ...
    def _build_mot_attention_masks(
        ...
        total_seq_len = video_seq_len + action_seq_len
        base_mask = torch.zeros((total_seq_len, total_seq_len), dtype=torch.bool, device=device)
        base_mask[:video_seq_len, :video_seq_len] = self._build_video_attention_mask(
            video_seq_len=video_seq_len,
            video_tokens_per_frame=video_tokens_per_frame,
            device=device,
        )
        base_mask[video_seq_len:, video_seq_len:] = True

        attention_masks = []
        for layer_idx in range(int(self.mot.num_layers)):
            layer_mask = base_mask.clone()
            if layer_idx in self._mot_action_video_attention_layer_set:
                layer_mask[video_seq_len:, :video_seq_len] = True
            attention_masks.append(layer_mask)
    ...
    def infer_joint(
        ...
        if test_action_with_infer_action:
            logger.warning(
                "`JointWAM.infer_joint` ignores `test_action_with_infer_action=True` "
    ...
    @torch.no_grad()
    def infer_action_one_pass_future_cache(
        self,
        prompt: Optional[str],
        input_image: torch.Tensor,
        action_horizon: int,
        ...
        infer_timesteps_video, _ = self.infer_video_scheduler.build_inference_schedule(
            num_inference_steps=num_inference_steps,
            device=self.device,
            dtype=latents_video.dtype,
            shift_override=sigma_shift,
