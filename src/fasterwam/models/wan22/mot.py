...
class MoT(nn.Module):
    ...
    def _resolve_attention_masks(
        ...
        for layer_idx, layer_mask in enumerate(attention_masks):
            ...
            if layer_mask.ndim != 2:
                raise ValueError(
                    f"`attention_mask[{layer_idx}]` must be 2D [S,S], got shape {tuple(layer_mask.shape)}"
                )
            if layer_mask.shape[0] != layer_mask.shape[1]:
                raise ValueError(
                    f"`attention_mask[{layer_idx}]` must be square, got shape {tuple(layer_mask.shape)}"
                )
            if layer_mask.shape[0] != expected_seq_len:
                raise ValueError(
                    f"`attention_mask[{layer_idx}]` seq length mismatch: "
                    f"mask={layer_mask.shape[0]} vs expected_total={expected_seq_len}"
                )
    ...
    def forward(
        ...
        for layer_idx in range(self.num_layers):
            ...
            layer_attention_mask = attention_masks[layer_idx]
            if layer_attention_mask.shape[0] != total_seq:
                raise ValueError(
                    "Attention mask seq length mismatch: "
                    f"mask={layer_attention_mask.shape[0]} vs tokens={total_seq}"
                )
