...
DEFAULT_SPARSE_MOT_CONDITION_LAYERS = (0, 4, 8, 12, 16, 20, 24, 28)


class ActionOnlyDiTBlock(nn.Module):
    """Action refinement block without text/context cross-attention."""
    ...
    def forward(
        ...
        x = self.gate(x, gate_msa, self.self_attn(attn_input, freqs, self_attn_mask=self_attn_mask))
...
class SparseActionDiT(nn.Module):
    ...
    def __init__(
        ...
        self.num_heads = num_heads
        self.noncondition_num_heads = noncondition_num_heads
        self.attn_head_dim = attn_head_dim
        self.condition_layers = self._normalize_condition_layers(condition_layers, num_layers)
        self.condition_layer_set = frozenset(self.condition_layers)

        if num_heads <= 0:
            raise ValueError(f"`num_heads` must be > 0, got {num_heads}")
        if noncondition_num_heads <= 0:
            raise ValueError(f"`noncondition_num_heads` must be > 0, got {noncondition_num_heads}")
        if attn_head_dim <= 0:
        ...
        self.blocks = nn.ModuleList(
            [
                ...
                else ActionOnlyDiTBlock(
                    hidden_dim=hidden_dim,
                    attn_head_dim=attn_head_dim,
                    num_heads=noncondition_num_heads,
                    ffn_dim=ffn_dim,
                    eps=eps,
                )
    ...
    def from_pretrained(
        ...
        if action_dit_config is None:
            raise ValueError("`action_dit_config` is required for SparseActionDiT.from_pretrained().")
        if skip_dit_load_from_pretrain:
            logger.info(
                "Skipping SparseActionDiT pretrained load (`skip_dit_load_from_pretrain=True`); "
        ...
        if not action_dit_pretrained_path:
            logger.info("No `action_dit_pretrained_path` provided, initializing SparseActionDiT with random weights.")
            return cls(**action_dit_config).to(device=device, dtype=torch_dtype)
        ...
        if policy:
            logger.info(f"SparseActionDiT backbone payload policy: {policy}")
        ...
        expected_meta = {
            ...
            "condition_layers": list(action_expert.condition_layers),
        }
        for key in cls.ACTION_BACKBONE_META_KEYS:
            ...
            elif key == "condition_layers":
                if tuple(got_value) != tuple(expected_value):
                    raise ValueError(
                        f"`meta.{key}` mismatch in {action_dit_pretrained_path}: "
                        f"expected {expected_value}, got {got_value}"
                    )
        ...
        if missing_keys or unexpected_keys:
            raise ValueError(
                "SparseActionDiT backbone key mismatch in preprocessed payload. "
        ...
        logger.info(
            "Loaded SparseActionDiT backbone from %s (keys=%d; random_kept_prefixes=%s).",
