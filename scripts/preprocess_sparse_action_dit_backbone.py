...
from fasterwam.models.wan22.sparse_action_dit import (
    SparseActionDiT,
    DEFAULT_SPARSE_MOT_CONDITION_LAYERS,
)
from fasterwam.models.wan22.helpers.loader import load_wan22_ti2v_5b_components
...
def _parse_condition_layers(value: Any) -> tuple[int, ...]:
    if value is None:
        return tuple(DEFAULT_SPARSE_MOT_CONDITION_LAYERS)
    if isinstance(value, str):
        value = value.strip()
        if not value:
            raise ValueError("`condition_layers` string cannot be empty.")
        if value.startswith("["):
    ...
    if not isinstance(value, (list, tuple)):
        raise ValueError(f"`condition_layers` must be list/tuple or comma string, got {type(value)}.")

    out = []
    seen = set()
    for raw_idx in value:
        if isinstance(raw_idx, bool) or not isinstance(raw_idx, int):
            if isinstance(raw_idx, str) and raw_idx.strip().lstrip("-").isdigit():
                ...
                raise ValueError(f"`condition_layers` entries must be integers, got {raw_idx!r}.")
        if raw_idx in seen:
            raise ValueError(f"`condition_layers` contains duplicate layer index {raw_idx}.")
        seen.add(raw_idx)
        out.append(int(raw_idx))
    if out != sorted(out):
...
def _load_model_config(path: Path) -> tuple[dict[str, Any], dict[str, Any], Any]:
    ...
    for key in ["num_heads", "attn_head_dim", "text_dim", "freq_dim"]:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Preprocess SparseActionDiT backbone weights from WanVideoDiT."
    )
    parser.add_argument("--model-config", required=True, help="Path to the FasterWAM model yaml.")
    parser.add_argument("--output", required=True, help="Output .pt path for preprocessed SparseActionDiT backbone.")
    parser.add_argument("--device", default="cpu", help="Device for loading model and preprocessing.")
    parser.add_argument("--dtype", default="float32", choices=["float32", "float16", "bfloat16"])
    parser.add_argument(
        "--condition-layers",
        default=None,
        help="Comma list or YAML list of condition layer indices. Defaults to the config value, then the SparseMoT default.",
    )
    parser.add_argument(
        "--apply-alpha-scaling",
        default="true",
    ...
    cfg_condition_layers = action_cfg.get("condition_layers", None)
    if _is_unresolved_interpolation(cfg_condition_layers):
        cfg_condition_layers = cfg.get("condition_layers", None)
    if cfg_condition_layers is None:
        cfg_condition_layers = cfg.get("condition_layers", None)
    condition_layers = _parse_condition_layers(
        args.condition_layers if args.condition_layers is not None else cfg_condition_layers
    )
    ...
    action_cfg["condition_layers"] = list(condition_layers)

    print(
        f"[INFO] Loaded model config from {model_config_path}. "
        f"Preprocessing SparseActionDiT backbone with dtype={torch_dtype} on device={args.device}, "
        f"condition_layers={list(condition_layers)}, apply_alpha_scaling={apply_alpha_scaling}."
    )
    ...
    if int(action_cfg["num_layers"]) != video_num_layers:
        raise ValueError(
            "SparseActionDiT `num_layers` must match video expert: "
            f"{action_cfg['num_layers']} vs {video_num_layers}."
        )
    ...
    if int(action_cfg["num_heads"]) * int(action_cfg["attn_head_dim"]) != int(video_expert.hidden_dim):
        raise ValueError(
            "SparseActionDiT condition-layer attention dim must equal video attention dim: "
            f"{action_cfg['num_heads']} * {action_cfg['attn_head_dim']} != {video_expert.hidden_dim}."
        )

    action_expert = SparseActionDiT(**action_cfg).to(device=args.device, dtype=torch_dtype)
    action_state = action_expert.state_dict()
    video_state = video_expert.state_dict()
    backbone_keys = SparseActionDiT.backbone_key_set(action_state.keys())
    ...
    payload = {
        "policy": {
            "skip_prefixes": list(SparseActionDiT.ACTION_BACKBONE_SKIP_PREFIXES),
            ...
            "noncondition_num_heads": int(action_cfg["noncondition_num_heads"]),
            "attn_head_dim": int(action_cfg["attn_head_dim"]),
            "source_video_num_layers": video_num_layers,
        },
    ...
    print(
        "[INFO] Saved SparseActionDiT backbone payload to "
